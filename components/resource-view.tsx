...
interface ResourceViewProps {
  ...
  onSelectSlot: (slotInfo: { start: Date; end: Date; collaborateurId: string }) => void
  onDuplicateEvent: (event: Evenement, newDate: Date, newCollaborateurId?: string, isCollaborateurChange?: boolean) => void
...
export default function ResourceView({
  ...
  const [dragOverCell, setDragOverCell] = useState<{ collaborateurId: string; dayIndex: number } | null>(null)
  ...
  const handleCellClick = useCallback(
    (collaborateurId: string, day: Date) => {
      ...
      const canCreate =
        userRole === "admin" ||
        userRole === "manager" ||
        (userRole === "collaborateur" && collaborateurId === userCollaborateurId)
  ...
  const handleDragStart = useCallback(
    (event: Evenement) => {
      ...
      const canDrag =
        userRole === "admin" ||
        userRole === "manager" ||
        (userRole === "collaborateur" && userCollaborateurId && event.collaborateurId === userCollaborateurId)
  ...
  const handleDragOver = useCallback((e: React.DragEvent, collaborateurId: string, dayIndex: number) => {
    e.preventDefault()
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, collaborateurId: string, day: Date) => {
      ...
      if (draggedEvent) {
        ...
        if (canDrop) {
          // Si c'est le même collaborateur, c'est juste un changement de date
          // Sinon, c'est un changement de collaborateur (nécessite un traitement spécial)
          const isCollaborateurChange = draggedEvent.collaborateur_id !== collaborateurId && 
                                        draggedEvent.collaborateurId !== collaborateurId;
                                        
          onDuplicateEvent(draggedEvent, day, collaborateurId, isCollaborateurChange);
        }
      }
              ...
              <tbody>
                {collaborateurs.map((collaborateur) => (
                  <tr key={collaborateur.id}>
                    <td className="border p-2 font-medium" style={{ color: collaborateur.couleur }}>
                      {collaborateur.nom}
                    </td>
                    {weekDays.map((day, dayIndex) => {
                      const eventsForDay = getEventsForCollaboratorAndDay(collaborateur.id, day)

                      // Vérifier si l'utilisateur peut interagir avec cette cellule
                      const canInteract =
                        userRole === "admin" ||
                        userRole === "manager" ||
                        (userRole === "collaborateur" && collaborateur.id === userCollaborateurId)

                      const isHighlighted =
                        dragOverCell?.collaborateurId === collaborateur.id && dragOverCell?.dayIndex === dayIndex

                      return (
                        <td
                          key={dayIndex}
                          className={`border p-1 align-top h-24 relative ${
                            canInteract && eventsForDay.length === 0 ? "cursor-pointer hover:bg-muted/20" : ""
                          } ${isHighlighted ? "bg-blue-100" : ""}`}
                          onClick={() =>
                            canInteract && eventsForDay.length === 0 ? handleCellClick(collaborateur.id, day) : null
                          }
                          onDragOver={(e) => canInteract && handleDragOver(e, collaborateur.id, dayIndex)}
                          onDragLeave={() => canInteract && handleDragLeave()}
                          onDrop={(e) => canInteract && handleDrop(e, collaborateur.id, day)}
                        >
                                  ...
                                  <div
                                    ...
                                    className={`p-1 rounded text-xs group relative ${
                                      ...
                                      event.typeEvenement === "absence"
                                        ? getAbsenceBackgroundColor(event.typeAbsence)
                                        : `bg-[${collaborateur.couleur}20]`
                                    } ${event.verrouille ? "border border-gray-400" : ""}`}
                                    style={
                                    ...
                                    draggable={canEdit}
                                    onDragStart={() => canEdit && handleDragStart(event)}
                                  >
                                    ...
                                    {userRole === "admin" && (
                                      <button
                                        ...
                                        onClick={(e) => handleToggleLock(event, e)}
                                        title={
                                          event.verrouille ? "Déverrouiller l'événement" : "Verrouiller l'événement"
                                        }
                                      >
                                        {event.verrouille ? (
                                    ...
                                    {event.typeEvenement === "presence" ? (
                                      <>
                                        <div className="font-medium">{event.lieuChantier}</div>
                                        <div className="text-muted-foreground">
                                          {formatTime(event.start)} - {formatTime(event.end)}
                                        </div>
                                        <div className="flex flex-wrap gap-1 mt-1">
                                          {event.grandDeplacement && (
                                            <span className="bg-blue-100 text-blue-800 px-1 rounded">GD</span>
                                          )}
                                          {event.panierRepas && (
                                            <span className="bg-green-100 text-green-800 px-1 rounded">PR</span>
                                          )}
                                          {event.prgd && (
                                          ...
                                          {(event.heuresSupplementaires || 0) > 0 && (
                                            <span className="bg-orange-100 text-orange-800 px-1 rounded">
                                              HS: {event.heuresSupplementaires || 0}h
                                            </span>
                                          )}
                                        </div>
                                      </>
                                      ...
                                      <>
                                        <div
                                          className={`font-medium ${
                                            event.typeAbsence ? getAbsenceTextColor(event.typeAbsence) : ""
                                          }`}
                                        >
                                          {event.typeAbsence}
                                        </div>
