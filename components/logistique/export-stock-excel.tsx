"use client"

import { useState } from "react"
import { Button } from "@/components/ui/button"
import { FileDown } from "lucide-react"
import type { StockItem } from "@/lib/logistique-types"

interface ExportStockExcelProps {
  stockItems: StockItem[]
}

export default function ExportStockExcel({ stockItems }: ExportStockExcelProps) {
  const [isExporting, setIsExporting] = useState(false)

  const handleExport = async () => {
    try {
      setIsExporting(true)

      // Préparer les données pour l'export
      const data = stockItems.map((item) => {
        return {
          "Type de câble": item.cableType,
          "Type de module": item.typeM,
          "Type de fibre": item.typeG,
          Enroulement: item.enroulement,
          "Longueur (m)": item.longueur,
          Description: `${item.cableType} ${item.typeM} ${item.typeG}`,
        }
      })

      // Convertir les données en CSV
      const headers = Object.keys(data[0] || {}).join(",")
      const rows = data.map((row) =>
        Object.values(row)
          .map(
            (value) =>
              // Entourer les valeurs de guillemets pour gérer les virgules dans les valeurs
              `"${String(value).replace(/"/g, '""')}"`,
          )
          .join(","),
      )
      const csv = [headers, ...rows].join("\n")

      // Créer un blob et télécharger le fichier
      const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" })
      const url = URL.createObjectURL(blob)
      const link = document.createElement("a")
      link.setAttribute("href", url)

      const today = new Date()
      const formattedDate = today.toLocaleDateString("fr-FR").replace(/\//g, "-")
      link.setAttribute("download", `Stock_Cables_${formattedDate}.csv`)

      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
    } catch (error) {
      console.error("Erreur lors de l'export Excel:", error)
    } finally {
      setIsExporting(false)
    }
  }

  return (
    <Button onClick={handleExport} disabled={isExporting} className="flex items-center gap-2">
      <FileDown className="h-4 w-4" />
      {isExporting ? "Export en cours..." : "Exporter en Excel"}
    </Button>
  )
}
