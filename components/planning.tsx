...
import { CalendarIcon, Users, Building, AlertCircle } from "lucide-react"
...
import type { Evenement, Role, Collaborateur } from "@/lib/types"
...
export default function Planning({ userRole, userCollaborateurId, user }: PlanningProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const { data: collabData, error: collabError } = await supabase.from("collaborateurs").select("*").order("nom")

        if (collabError) {
          throw new Error(`Erreur lors du chargement des collaborateurs: ${collabError.message}`)
        }
        ...
        const { data: eventData, error: eventError } = await supabase.from("evenements").select(`
            *,
        ...
        if (eventError) {
          throw new Error(`Erreur lors du chargement des événements: ${eventError.message}`)
        }
        ...
        if (selectedCollaborateurs.length === 0) {
          ...
          console.log("Collaborateurs sélectionnés initialisés:", initialSelectedCollabs.length)
        }
        ...
        toast({
          ...
          description: error instanceof Error ? error.message : "Impossible de charger les données du planning.",
          variant: "destructive",
        })
  ...
  const handleSaveEvent = useCallback(
    async (event: Evenement) => {
      ...
      if (canSave) {
        try {
          ...
          if (!collaborateur) {
            throw new Error(`Collaborateur avec ID ${event.collaborateur_id} non trouvé`)
          }
          ...
          if (selectedEvent && events.find((e) => e.id === event.id)) {
            ...
            const { error } = await supabase.from("evenements").update(eventData).eq("id", event.id)
            ...
            setEvents(events.map((e) => (e.id === event.id ? { ...event, collaborateur } : e)))
          ...
          toast({
            title: "Erreur",
            description: error instanceof Error ? error.message : "Impossible de sauvegarder l'événement.",
            variant: "destructive",
          })
        }
      }
    ...
    [events, selectedEvent, userRole, userCollaborateurId, toast, supabase, collaborateurs],
  )
  ...
  const handleDuplicateEvent = useCallback(
    async (event: Evenement, newDate: Date, newCollaborateurId?: string) => {
      ...
      const existingEvent = events.find((e) => {
        ...
        return (
          e.collaborateur_id === (newCollaborateurId || event.collaborateur_id) &&
      ...
      try {
        ...
        const collaborateur = collaborateurs.find((c) => c.id === targetCollabId)
        if (!collaborateur) {
          throw new Error(`Collaborateur avec ID ${targetCollabId} non trouvé`)
        }

        // Créer une copie de l'événement avec un nouvel ID
        const startTime = new Date(event.start).getHours() * 60 + new Date(event.start).getMinutes()
        const endTime = new Date(event.end).getHours() * 60 + new Date(event.end).getMinutes()
        ...
        toast({
          title: "Erreur",
          description: error instanceof Error ? error.message : "Impossible de dupliquer l'événement.",
          variant: "destructive",
        })
      }
    },
  ...
  const handleToggleLock = useCallback(
    async (eventId: string, locked: boolean) => {
      ...
      try {
        ...
        const { error } = await supabase.from("evenements").update({ verrouille: locked }).eq("id", eventId)
        ...
        console.error("Erreur lors du verrouillage/déverrouillage de l'événement:", error)
        toast({
          title: "Erreur",
  ...
  }, [userRole, userCollaborateurId, filteredCollaborateurs, user?.collaborateursGeres, collaborateurs])
  ...
  if (collaborateurs.length === 0) {
    return (
      <Alert variant="warning" className="mb-6">
        ...
        <AlertDescription>
          Aucun collaborateur n'a été trouvé dans la base de données. Veuillez ajouter des collaborateurs avant
          d'utiliser le planning.
          <div className="mt-2">
            <Button variant="outline" onClick={() => (window.location.href = "/test-supabase")} className="mr-2">
