...
export function usePlanningSync() {
  ...
  const addPlanningEvent = async (event: Omit<PlanningEvent, "id">) => {
    try {
      ...
      const collaborateurExists = collaborateurs.some((c) => c.id === event.collaborateurId)
      if (!collaborateurExists) {
        throw new Error(`Le collaborateur avec l'ID ${event.collaborateurId} n'existe pas`)
      }
  ...
  const updatePlanningEvent = async (id: string, updates: Partial<PlanningEvent>) => {
    try {
      setIsLoading(true)
      ...
      if (updates.collaborateurId) updateData.collaborateur_id = updates.collaborateurId

      const { data, error } = await supabase.from("planning_events").update(updateData).eq("id", id).select()
  ...
  const deletePlanningEvent = async (id: string) => {
    try {
      ...
      setPlanningEvents((prevEvents) => prevEvents.filter((event) => event.id !== id))
  ...
  const checkSynchronization = () => {
    const eventsWithInvalidCollaborateurs = planningEvents.filter(
      (event) => !collaborateurs.some((collab) => collab.id === event.collaborateurId),
    )
