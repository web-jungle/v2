...
const collaborateursInitiaux = [
  { id: "1", nom: "ARTES Damien", couleur: "#3174ad", entreprise: "ORIZON TELECOM" },
  { id: "2", nom: "BENAMARA Walid", couleur: "#e6550d", entreprise: "ORIZON TELECOM" },
  { id: "3", nom: "BELTRAN Noah", couleur: "#31a354", entreprise: "ORIZON INSTALLATION" },
  { id: "4", nom: "BERRAHMOUNE Abdelkader", couleur: "#756bb1", entreprise: "ORIZON INSTALLATION" },
  { id: "5", nom: "BLOT Alexia", couleur: "#636363", entreprise: "ORIZON TELECOM" },
...
export async function seedDatabase() {
  ...
  for (const collaborateur of collaborateursInitiaux) {
    const { error } = await supabase.from("collaborateurs").upsert(collaborateur, { onConflict: "id" })
  ...
  for (const utilisateur of utilisateursInitiaux) {
    const { error } = await supabase.from("utilisateurs").upsert(utilisateur, { onConflict: "id" })
