...
export async function GET() {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des éléments de stock:",
      error
    );
    return NextResponse.json(
      { error: "Erreur lors de la récupération des éléments de stock" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "Erreur lors de la création de l'élément de stock" },
      { status: 500 }
    );
  }
}
