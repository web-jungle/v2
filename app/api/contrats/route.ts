...
export async function GET() {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des contrats de maintenance:",
      error
    );
    return NextResponse.json(
      { error: "Erreur lors de la récupération des contrats de maintenance" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error(
      "Erreur lors de la création du contrat de maintenance:",
      error
    );
    return NextResponse.json(
      { error: "Erreur lors de la création du contrat de maintenance" },
      { status: 500 }
    );
  }
}
