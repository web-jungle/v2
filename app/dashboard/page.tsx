...
import { CalendarIcon, Clock, MapPin, Users, TrendingUp, Coffee, Car, Home, Building, FileText } from "lucide-react"
...
export default function DashboardPage() {
  ...
  const [timeframe, setTimeframe] = useState<"semaine" | "mois" | "annee">("mois")
  ...
  useEffect(() => {
    if (user && user.role === "collaborateur") {
      ...
      toast({
        title: "Accès limité",
        description: "Le tableau de bord complet est réservé aux administrateurs et managers.",
        variant: "destructive",
      })
    }
  }, [user, router, toast])
  ...
  const totalHeuresSupp = filteredEvents.reduce((sum, event) => sum + event.heuresSupplementaires, 0)
  const totalGrandsDeplacement = filteredEvents.filter((event) => event.grandDeplacement).length
  const totalPaniersRepas = filteredEvents.filter((event) => event.panierRepas).length
  const totalPRGD = filteredEvents.reduce((sum, event) => sum + (event.prgd ? event.nombrePRGD : 0), 0)

  // Données pour le graphique des événements par collaborateur
  const eventsByCollaborateur = filteredCollaborateurs
    .map((collaborateur) => {
      const count = filteredEvents.filter((event) => event.collaborateurId === collaborateur.id).length
      return {
  ...
  const eventTypes = [
    {
      name: "Standards",
      value: filteredEvents.filter((e) => !e.grandDeplacement && !e.panierRepas && !e.prgd).length,
      color: "#4f46e5",
    },
    {
  ...
  const zoneTrajetData = filteredEvents.reduce(
    (acc, event) => {
      if (event.zoneTrajet) {
  ...
  const eventsByEnterprise = entreprises.map((entreprise) => {
    const collaborateursIds = collaborateurs.filter((c) => c.entreprise === entreprise).map((c) => c.id)

    const count = events.filter((event) => {
      const eventDate = new Date(event.start)
      const isInTimeframe = eventDate >= new Date(new Date().setMonth(new Date().getMonth() - 1))
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {user.role === "admin" && (
          <Button variant="outline" className="flex items-center gap-2" onClick={() => router.push("/admin/recap")}>
            <FileText className="h-4 w-4" />
            Récapitulatif administratif
          </Button>
        )}
        <div className="flex flex-wrap gap-4">
          <Select value={selectedEnterprise || ""} onValueChange={(value) => setSelectedEnterprise(value || null)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Événements planifiés</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Grands déplacements</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Paniers repas</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="col-span-1">
          <CardHeader>
            ...
            <CardDescription>Top 10 des collaborateurs avec le plus d'événements planifiés</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="col-span-1">
          <CardHeader>
            ...
            <CardDescription>Répartition des événements par zone de trajet</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
        ...
        <Card className="col-span-1 lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Répartition des événements par entreprise (30 derniers jours)</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="col-span-1 lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>Récapitulatif des indicateurs clés</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Lieux de chantier</p>
                  <p className="text-lg font-semibold">{new Set(filteredEvents.map((e) => e.lieuChantier)).size}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">PRGD</p>
                  <p className="text-lg font-semibold">{totalPRGD}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <div className="p-2 bg-orange-500/10 rounded-full">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Moyenne d'heures supp.</p>
                  <p className="text-lg font-semibold">
                    {totalEvents > 0 ? (totalHeuresSupp / totalEvents).toFixed(1) : 0}h / événement
                  </p>
                </div>
              </div>
              {selectedEnterprise && (
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold">{selectedEnterprise}</p>
                  </div>
                </div>
              )}
            </div>
